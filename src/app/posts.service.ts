...
export class PostsService {
  ...
  getPosts(): Observable<Post[]> {
    const url = `${this.baseUrl}/posts?order=id.desc&select=*,user:users(username)`;
    return this.http.get<Post[]>(url);
  }

  newPost(postData: { body: string, user_id: number }): Observable<Post> {
    const url = `${this.baseUrl}/posts?select=*,user:users(username)`;
  ...
  likePost(id: number): Observable<Post> {
    const url = `${this.baseUrl}/rpc/like_post?select=*,user:users(username)`;
  ...
  repostPost(id: number): Observable<Post> {
    const url = `${this.baseUrl}/rpc/repost_post?select=*,user:users(username)`;
    return this.http.post<Post>(
      url, { params: { id_param: id } }
    );
  }
